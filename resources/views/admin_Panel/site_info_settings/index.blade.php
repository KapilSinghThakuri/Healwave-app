                        ...
                        <tbody>
                            @foreach ($siteinfos as $siteinfo)
                                <tr>
                                    ...
                                    <td>{{ $siteinfo->icon ?? '-' }}</td>
                                    <td>
                                        <a href="{{ route('websiteinfo.edit', ['websiteinfo' => $siteinfo->id]) }}"
                                            style="font-size: 20px;" title="Click For Edit"><i
                                                class="fa fa-pencil-square-o mr-2" aria-hidden="true"></i></a>
                                        <a href="#" data-id="{{ $siteinfo->id }}" data-toggle="modal"
                                            data-target="#delete_info_{{ $siteinfo->id }}"
                                            style="font-size: 25px; color: red;" title="Click For Delete"><i
                                                class="fa fa-trash-o" aria-hidden="true"></i></a>
                                    </td>
                                </tr>
                                <div id="delete_info_{{ $siteinfo->id }}" class="modal fade delete-modal"
                                    role="dialog">
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-body text-center">
                                                ...
                                                <div class="m-t-20 d-flex justify-content-center"> <a href="#"
                                                        class="btn btn-white mr-2" data-dismiss="modal">Close</a>
                                                    <form
                                                        action="{{ route('websiteinfo.destroy', ['websiteinfo' => $siteinfo->id]) }}"
                                                        method="POST">
                                                        @method('DELETE')
                                                        @csrf
